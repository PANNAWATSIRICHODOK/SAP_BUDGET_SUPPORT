...
begin
	...
	*/	
	...
	Declare cursor loopje for
		...
		Where (T0."Debit"-T0."Credit") <> 0 AND T0."TransType" IN( '30')	AND IFNULL(T1."U_NDBS_UseBudget",'N') = 'Y'  AND T0."TransId" = :datakey
		...
		INNER Join "@NDBS_BGC_OBGP" T5 ON T4."Code" = T5."Code"
		LEFT Join OPRC T6 ON T6."PrcCode" = T0."ProfitCode"
		Where (T0."Debit"-T0."Credit") <> 0 AND T0."TransType" IN('46','24') AND T0."TransId" = :datakey
		
		UNION ALL 
		
		
		-- Seperate GI,GR in JE Doc 24032026
		SELECT T0."RefDate"
		,CASE WHEN T0."TransType" = 30 THEN T0."U_NDBS_BudgetYear" ELSE YEAR(T0."RefDate") END AS "U_NDBS_BudgetYear"
		,T0."ProfitCode" "DocDept", T0."Project", (T0."Debit"-T0."Credit") "Amt",T0."TransId",T0."Line_ID",
				T5."Code",T5."U_Locked",T0."U_NDBS_NotCheckBudget",T0."U_NDBS_BudgetReason",T0."TransType" "BaseType",0 "BaseEntry",0 "BaseLine",
		...
		INNER Join "@NDBS_BGC_OBGP" T5 ON T4."Code" = T5."Code"
		LEFT Join OPRC T6 ON T6."PrcCode" = T0."ProfitCode"
		INNER JOIN (Select 'GR' AS "Type",T0."TransId" FROM OIGN T0 INNER JOIN IGN1 T1 ON T0."DocEntry"=T1."DocEntry" where T1."BaseType" ='-1' 
		UNION ALL
		Select 'GI' AS "Type",T0."TransId" FROM OIGE T0 INNER JOIN IGE1 T1 ON T0."DocEntry"=T1."DocEntry" where T1."BaseType" ='-1'
		)G1 ON T0."TransId"=G1."TransId"
		Where (T0."Debit"-T0."Credit") <> 0 AND T0."TransType" IN('59','60') AND T0."TransId" = :datakey;
		...
		--AND IFNULL(T1."U_NDBS_UseBudget",'N') = 'Y';	
	/*
	Declare cursor loopcap for
		SELECT T1."DocDate",T0."U_NDBS_BudgetYear",T7."OcrCode" "DocDept",T8."Project", 
		T0."LineTotal",T1."DocEntry",T0."LineNum",
		T6."Code",T6."U_Locked",
		Case When T6."U_Center" = 'Y' THEN T6."U_Department" 
		When T6."U_UseGroupCode" = 'N' THEN T9."PrcCode"
		ELSE T9."U_NDBS_BudgetDept" END "OcrCode"
		FROM ACQ1 T0 Inner Join OACQ T1 ON T0."DocEntry" = T1."DocEntry"
		Inner Join OITM T2 ON T2."ItemCode" = T0."ItemCode"
		Inner Join OADT T3 ON T2."AssetClass" = T3."Code"
		Inner Join OACT T4 ON T3."BalanceAct" = T4."AcctCode"
		Inner Join "@NDBS_BGC_BGPL" T5 ON T5."U_AccountCode" = T4."AcctCode"
		Inner Join "@NDBS_BGC_OBGP" T6 ON T5."Code" = T6."Code"
		Left JOin (select "ItemCode","OcrCode" ,RANK() OVER ( PARTITION BY "ItemCode","ValidFrom" ORDER BY "ItemCode","ValidFrom" DESC ) 
		AS "Rank" From ITM6 ) T7 ON T0."ItemCode" = T7."ItemCode" AND T7."Rank" = 1
		Left JOin (select "ItemCode","Project" ,RANK() OVER ( PARTITION BY "ItemCode","ValidFrom" ORDER BY "ItemCode","ValidFrom" DESC ) 
		AS "Rank" From ITM5 ) T8 ON T0."ItemCode" = T8."ItemCode" AND T8."Rank" = 1
		Inner Join OPRC T9 ON T9."PrcCode" = T7."OcrCode"
		Where T0."LineTotal" <> 0 AND T0."DocEntry" =  :datakey;	
*/	
-- Budget Contrrol
	error = 0;
	error_message = '';
	/*
	IF ( :object_type in ('1470000113','22','20','21','19','18','1470000049') )
	THEN
		IF ( :object_type = '1470000113') then
		    ...
		    FROM  OPRQ a inner join PRQ1 b on a."DocEntry" = b."DocEntry"
								...
								and A."DocEntry" = :datakey; 
		 
		    IF :LineNum > 0 Then
				error = -30;
		        error_message = 'Please define Department';
			end if;
		    ...
		    IF (:LineNum > 0) then
				error = -31;
		        error_message = 'Please define Budget Year ';
			end if;
		end if;
	
		IF ( :object_type = '1470000049') then
		    SELECT IFNULL(COUNT(LineNum),0) Into LineNum
		    FROM  OACQ a inner join ACQ1 b on a."DocEntry" = b."DocEntry"
		    inner join ITM6 c ON b."ItemCode" = c."ItemCode"
		                        WHERE (C."OcrCode" = '' or C."OcrCode" is null)
								and A."DocEntry" = :datakey; 
		 
		    IF :LineNum > 0 Then
				error = -30;
		        error_message = 'Please define Department on Fixed Asset';
			end if;
		...
		IF ( :object_type = '22') then
		    ...
		    IF :LineNum > 0 Then
				error = -30;
		        error_message = 'Please define Department';
		        --error_message = CONCAT('Please define Department');
			end if;
		    ...
		    IF (:LineNum > 0) then
				error = -30;
				--error_message = CONCAT('Please define Budget Year ',:LineNum);
		        error_message = 'Please define Budget Year';
			end if;
		end if;
	...
	END IF;
	*/
	Insert Into NDBS_STORE_TABLE
	Values
	(:object_type,:transaction_type,:error,'',:datakey);	
	if :error = 0 then
		IF ( :object_type in ('30','22','21','20','18','19'/*,'1470000049'*/)) then
				...
				if (currloop."Project" IS NULL) OR (currloop."Project" = '') then
					...
					if(:transaction_type in ('U','A')) then
						...
						IF BCount <> 0 Then
							...
							Select TOP 1 (T0."LineTotal" - 0), 0,T0."LineTotal" into BDiffAmount, BaseAmount,LineAmount
							FROM POR1 T0 
							--left JOin PRQ1 T1 ON T0."BaseEntry" = T1."DocEntry" AND T0."BaseLine" = T1."LineNum"
							--- 24 Oct 2025 ---
							LEFT Join OITM I1 ON T0."ItemCode"=I1."ItemCode"
							...
							*/
									...
									/*
									SELECT Case When T4."U_Center" = 'Y' THEN T4."U_Department" 
										When T4."U_UseGroupCode" = 'Y' THEN T5."U_NDBS_BudgetDept" 
										ELSE T5."PrcCode" END "OcrCode" into OldDept
									FROM PRQ1 T0 LEFT Join OPRQ T1 ON T0."DocEntry" = T1."DocEntry"
									LEFT Join OACT T2 ON T0."AcctCode" = T2."AcctCode"
									LEFT Join "@NDBS_BGC_BGPL" T3 ON T3."U_AccountCode" = T2."AcctCode"
									LEFT Join "@NDBS_BGC_OBGP" T4 ON T3."Code" = T4."Code"
									LEFT Join OPRC T5 ON T5."PrcCode" = T0."OcrCode"
									Where T0."DocEntry" = :BaseKey AND T0."LineNum" = :BaseLine;
									...
									VALUES
										(:AutoKey,:BCode,TO_NVARCHAR(:BYear),:OldDept,:BaseType,:BaseKey,:BaseLine,
											'22',:DocKey,:DocLine,-:BaseAmount,'R',:BValDate,'A','22',:DocKey,:DocLine);
							...
							--error = -32;
								--error_message = 'Department '+ :BDept+' Line No. '+TO_NVARCHAR(:DocLine) +' Budget Available '+
								--	TO_NVARCHAR(:BAvailable)+ ' less than Item Amount ' +TO_NVARCHAR(:BAmount);
							--error_message = CONCAT('Budget Department Not Found ',:BDept);
						end if;
					end if;
				end if;	
			end for;
	...
	elseif ( :object_type = '30') then
		...
		/*
	elseif ( :object_type = '1470000049') then
		if(:transaction_type IN ('A','C')) then
			Select IFNULL(MAX("DocEntry"),0) into AutoKey From "NDBS_BGC_OBDE";
			for currloop as loopcap do
				if (currloop."Project" IS NULL) OR (currloop."Project" = '') then
					BValDate = currloop."DocDate";
					BYear = currloop."U_NDBS_BudgetYear";
					BDept = currloop."OcrCode";
				...
				if (currloop."Project" <> '') then
					...
					if (:BProject <> '') then
						...
						if(:transaction_type in ('U','A')) then
							...
							/*
								Select TOP 1 (T0."LineTotal"-T1."LineTotal"), T1."LineTotal",T0."LineTotal" into BDiffAmount, BaseAmount,LineAmount
								FROM POR1 T0 left JOin PRQ1 T1 ON T0."BaseEntry" = T1."DocEntry" AND T0."BaseLine" = T1."LineNum"
								WHERE T0."DocEntry" = :DocKey AND T0."LineNum" = :DocLine;
								...
								IF ((:BAvailable+:BaseAmount) >= :LineAmount) OR (:BLocked = 'N') OR  (:BNotChecked = 'Y') then
									...
									--error_message = 'Project '+ :BProject+' Line No. '+TO_NVARCHAR(:DocLine) +' Budget Available '+
									--	TO_NVARCHAR(:BAvailable)+ ' less than Item Amount ' +TO_NVARCHAR(:BAmount);
									error_message = CONCAT(CONCAT('Project ',:BProject),'Over budget2');
...
elseif ( :object_type = '19') then
	...
	elseif ( :object_type = '30') then
		if(:transaction_type IN ('A','U')) then
			Select IFNULL(MAX("DocEntry"),0) into AutoKey From "NDBS_BGC_OBPE";
			/*
			Select CASE WHEN "StornoToTr" <> 0 THEN "C" ELSE "N" END into IsCancelled From OJDT Where "DocEntry" = :datakey;
			IF IsCancelled = 'C' then
				for currloop as loopcncancel do
					IF (currloop."Project" <> '') then
						BaseType = currloop."BaseType";
