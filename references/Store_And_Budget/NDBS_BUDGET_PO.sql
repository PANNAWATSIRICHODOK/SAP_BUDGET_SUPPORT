...
begin
	...
	Declare cursor looppo for
		...
		left join 
		( select T0."DocEntry",SUM(T0."LineTotal") AS "LineTotal" from POR1 T0 where T0."DocEntry" = :datakey group by  T0."DocEntry")S ON T0."DocEntry" =S."DocEntry"
	...
	Declare cursor looppoclose for
		...
		LEFT Join OITM I1 ON T0."ItemCode"=I1."ItemCode"
		left join 
		( select T0."DocEntry",SUM(T0."OpenSum") AS "OpenSum" from POR1 T0 where T0."DocEntry" = :datakey group by  T0."DocEntry")S ON T0."DocEntry" =S."DocEntry"
